...
2026-09-15。状态：重构实施中。可序列化流程已覆盖交互式事件检定和玩家对抗攻击；Modifier 已接管卡牌检定、治疗、移动、精神减伤、月光力量和伤害上限；Trigger 已接管狼毒/保护时钟、月光回血及 Before/AfterDamage；Action 注册表已接管特殊房间和主动物品；物品已具备兼容旧存档的实例 ID、独立使用状态和效果来源。敌人整轮行动、房间发现检定、作祟检定及剧本目标仍有兼容路径，引擎重构尚未完成。
...
| Action     | 可用性、文案与处理器 ID 共用定义；特殊房间和主动物品已迁移                          | 迁移封窗、治疗、攻击和剧本目标              |
| 物品实例   | definitionId / instanceId、独立使用状态、消耗指定副本、旧字符串存档兼容、主动注册表 | 增加转移、丢弃和带次数的非消耗状态          |
| 展示与联机 | 仍兼容现有 queue 与 revision                                                        | 结构化结果事件、按玩家投影和 commandId 去重 |

下一实施批次按依赖顺序推进：

1. 建立房间和人物轨道的有效规则视图，再迁移神秘电梯、坍塌房间和封窗，统一预览、合法性检查与执行参数。
2. 将敌人逐个移动/攻击迁入 Workflow，输出结构化移动、投骰和伤害事件。
3. 增加卡牌获得、进入房间和死亡时机，并补充物品转移、丢弃和次数状态。
4. 接入服务端玩家视图投影、commandId 去重与手机端抽屉布局。
...
这些是静态代码审查结论；并不意味着每一种潜在连锁错误都已在现有卡牌中复现。当前 141 项通过的测试是迁移基线，不代表未来效果组合已获得覆盖。
...
| 阶段              | 工作                                                                      | 完成标准                                                                           |
| ----------------- | ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| 0：固化行为       | 记录当前 141 项测试基线，保存关键局面与输入记录；区分既有规则和已确认错误 | 电梯、坍塌、感染、死亡、敌人阶段、联机骰子归属都有可重复验收用例；不靠截图判定规则 |
