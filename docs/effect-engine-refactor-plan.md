...
这些是静态代码审查结论；并不意味着每一种潜在连锁错误都已在现有卡牌中复现。当前 198 项测试是迁移基线，不代表未来效果组合已获得覆盖。
