...
2026-09-15。状态：基于当前工作区的代码审查与架构提案，尚未实施。界面和素材改动独立提交，本文档不代表引擎重构已经完成。
...
这些是静态代码审查结论；并不意味着每一种潜在连锁错误都已在现有卡牌中复现。现有 122 项通过的测试是迁移基线，不代表未来效果组合已获得覆盖。
...
| 阶段 | 工作 | 完成标准 |
| --- | --- | --- |
| 0：固化行为 | 记录当前 122 项测试基线，保存关键局面与输入记录；区分既有规则和已确认错误 | 电梯、坍塌、感染、死亡、敌人阶段、联机骰子归属都有可重复验收用例；不靠截图判定规则 |
