...
1. 将 StatusExpired、RoundStatusTick 等嵌套时机纳入可序列化父流程，再为同优先级触发器增加显式顺序选择。
...
这些是静态代码审查结论；并不意味着每一种潜在连锁错误都已在现有卡牌中复现。当前 204 项测试是迁移基线，不代表未来效果组合已获得覆盖。
