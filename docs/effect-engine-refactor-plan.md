...
2026-09-22 补充：四个剧本已接入 `lib/content/scenarios/` 生命周期注册表，统一初始化、组合触发、状态胜利检查、击败敌人回调、超时结果和敌人阶段收尾；正式 Workflow 与模拟适配器共用剧本收尾。物品能力及运行时规则校验已共用 `lib/content/item-abilities.mjs`，消除重复能力白名单。具体行动处理器、狼人状态推进等仍待逐步迁移，素材室草稿仍不进入对局。开发契约见 [剧本与物品机制扩展指南](content-extension-guide.md)。

2026-09-16。状态：重构实施中。可序列化流程已覆盖事件检定、玩家攻击、逐个敌人的移动攻击、普通移动与入房、探索落位与坠落抵达、物品/预兆获得、作祟检定、电梯、坍塌及剧本目标检定，并已支持按值分支、循环、嵌套暂停和可恢复选择请求；声明式反应会暂停后续效果，回答后从存档中的父流程续体恢复。检定已提供 BeforeCheck / AfterRoll / AfterCheck 时机；BeforeRoomEnter / AfterRoomEnter、BeforeCardGain / AfterCardGain、BeforeDamage / AfterDamage、BeforeDeath / AfterDeath，以及人物、整轮、作祟、状态到期和敌人行动开始时机均可保存恢复，并通过幂等标记避免重复移动、发牌、伤害、刷新和倒计时。状态到期会在 RoundStatusTick 的父流程内顺序派发，玩家反应结算完毕后才允许敌人规划移动。标记为 `playerOrder` 的同优先级触发器会生成由负责人物控制者回答的强制顺序选择，三项以上效果逐项确定，存档恢复、联机权限和服务端超时均沿用权威选择流程。尚未迁入 Workflow 的时机若请求暂停会原子回滚并报错，不再让父结算错误地继续执行。多人反应按当前座位开始的稳定顺序逐个响应，支持显式跳过；联机超时由服务端时钟验证并在轮询时执行声明的默认选择。正常试玩不再创建投骰试跑或动作捕获状态，生产路径中的 `_rollCapture` 与 `_rollReplay` 均已删除。Modifier 已接管卡牌检定、治疗、移动、精神减伤、月光力量和伤害上限；Trigger 已接管狼毒/保护时钟、月光回血、伤害、检定、卡牌获得、进入房间、死亡、人物/整轮起止、作祟开始和状态过期时机；Action 注册表已接管特殊房间、物品管理与使用、休息、封窗、治疗、攻击、狼群追猎指令和现有剧本目标，带检定的行动用同一份可序列化 checkSpec 向展示、Workflow 和处理器提供属性、类型、修正范围、加值与门槛。物品、地图、移动、抽牌、属性和状态均可通过白名单 Effect 操作，原子效果批次在任一步失败时回滚全部修改；生产中的狼毒感染与治疗也已改走 Status Effect，治疗以原子批次同时移除感染并添加保护。房间实例与人物属性轨道按剧本、阶段、人物和来源优先级解析有效规则视图。联机已增加 commandId 去重和玩家投影；负责人物的座位才能响应选择，牌堆顺序、随机种子、敌对阵营属性/物品/状态、私密事件、日志、反馈、卡牌请求和流程内部数据均不会投影给无权客户端；结构化事件已接入探索手记，敌人移动、投骰、属性、伤害、状态和物品展示开始直接消费事件。正式对局、快速测试和联机统一从 `createGame` 进入 Workflow；自动试玩代理也驱动相同的投骰、选择、落位、卡牌和伤害命令，旧的交互建局别名已经删除，确定性直算只通过显式 `createSimulationGame` 适配器供差分测试使用。v2/v3 存档通过显式迁移器升级至 v4，旧版待处理投骰只能回退到明确的未提交动作边界，无法安全回退则拒绝载入。现有内容的原子支付标注、重复展示字段及剩余兼容路径清理仍待完成，引擎重构尚未完成。

当前实现进度：

| 能力       | 已完成                                                                                                                                                                                                                                                               | 下一步                                           |
| ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------ |
| Workflow   | 流程定义注册表、JSON 流程帧、多次等待、按值/选择分支、循环与嵌套暂停；选择已接权威命令；所有现有交互投骰、移动、落位、入房、物品/预兆获得和生命周期入口均已迁移；多人响应按座位排序并可逐个跳过；生产入口与自动试玩默认使用 Workflow，确定性直算由具名模拟适配器隔离 | 收缩直算适配器和旧兼容入口                       |
...
| Trigger    | 稳定优先级、同一根动作限次；状态、月光、伤害、检定、卡牌获得、进入房间、死亡、回合、作祟和过期时机；反应窗口支持多响应者、保存恢复和服务端超时；同优先级效果可声明由负责人物逐项决定完整顺序                                                                         | 补充因果深度与循环诊断                           |
| Action     | 可用性、目标命令、文案、处理器与 Workflow 共用定义；特殊房间、物品、休息、封窗、治疗、攻击、狼群指令和剧本目标已迁移；内容检定由可序列化 checkSpec 统一属性、类型、修正、加值和门槛                                                                                  | 为新增内容继续声明成本与取消契约                 |
...
1. 逐步淘汰重复的结算日志拼接和旧展示字段；`enemyMovements` 仅保留旧存档显示回退。
2. 保留明确的差分测试入口，继续删除生产不可达的旧直算结算分支。
3. 要求后续涉及多步支付的内容使用原子 Effect 批次，统一取消与退款行为。
4. 在规则引擎边界稳定后继续手机端布局与真实设备验收。
...
这些是静态代码审查结论；并不意味着每一种潜在连锁错误都已在现有卡牌中复现。当前 210 项测试是迁移基线，不代表未来效果组合已获得覆盖。
...
| 阶段              | 工作                                                                                 | 完成标准                                                                           |
| ----------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------- |
| 0：固化行为       | 记录当前 198 项测试基线，保存关键局面与输入记录；区分既有规则和已确认错误            | 电梯、坍塌、感染、死亡、敌人阶段、联机骰子归属都有可重复验收用例；不靠截图判定规则 |
