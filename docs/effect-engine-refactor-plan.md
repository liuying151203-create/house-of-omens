...
2026-09-15。状态：重构实施中。可序列化流程已覆盖交互式事件检定和玩家对抗攻击；Modifier 已接管卡牌检定、治疗、移动、精神减伤、月光力量和伤害上限；Trigger 已接管狼毒/保护时钟、月光回血及 Before/AfterDamage；Action 注册表已接管神秘电梯、主动坠落和地下室回程；物品已具备兼容旧存档的实例 ID、独立使用状态和效果来源。敌人整轮行动、房间发现检定、作祟检定及剧本目标仍有兼容路径，引擎重构尚未完成。

当前实现进度：

| 能力       | 已完成                                                                             | 下一步                                             |
| ---------- | ---------------------------------------------------------------------------------- | -------------------------------------------------- |
| Workflow   | 流程定义注册表、JSON 流程帧、多次等待、事件检定、玩家攻击                          | 迁移电梯、坍塌、敌人逐个移动攻击和作祟检定         |
| Effect     | 白名单操作、事件参数引用、执行回执、每批步数上限；伤害调整、反伤、属性与状态操作   | 选择请求、移动实体、抽牌和地图事务操作             |
| Modifier   | sum/max/unique/replace/cap、来源追踪、卡牌和月光查询、剧本运行时修正、物品实例来源 | 地图拓扑和人物轨道的有效规则视图                   |
| Trigger    | 稳定优先级、同一根动作限次、状态倒计时/转化、月光回血、Before/AfterDamage          | 卡牌获得、进入房间和死亡时机                       |
| Action     | 可用性、文案与处理器 ID 共用定义；三个特殊房间行动已迁移                           | 迁移封窗、治疗、攻击、剧本目标和物品主动能力       |
| 物品实例   | definitionId / instanceId、独立使用状态、消耗指定副本、旧字符串存档兼容            | 将所有主动物品改为 Action + Effect，增加转移与丢弃 |
| 展示与联机 | 仍兼容现有 queue 与 revision                                                       | 结构化结果事件、按玩家投影和 commandId 去重        |

下一实施批次按依赖顺序推进：

1. 将现有主动物品迁入 Action + Effect，去除 `useItem` 对 movement / healPhysical / healMental 的固定枚举，并补充实例转移与丢弃操作。
2. 建立房间和人物轨道的有效规则视图，再迁移神秘电梯、坍塌房间和封窗，统一预览、合法性检查与执行参数。
3. 将敌人逐个移动/攻击迁入 Workflow，输出结构化移动、投骰和伤害事件。
4. 增加卡牌获得、进入房间、死亡时机，随后接服务端玩家视图投影与手机端抽屉布局。
...
| lib/game-engine.mjs:46、1762                              | _rollCapture 用全零骰子试跑动作，再以 _rollReplay 重跑                            | 如果后续步骤取决于真实骰点，试跑路径和实际路径可能不同；也不适合在途中插入反应窗口 |
...
| scripts/room-server.mjs:33                                | snapshot 返回完整 r.game，隐藏信息主要由 UI 过滤                                  | 后续秘密目标、手牌和隐藏敌人需要服务端视图投影；不能仅靠不渲染                     |
| app/page.jsx                                              | 审查时约 2145 行，游戏引擎约 1966 行                                              | 拆分有价值，但行数本身不是首要问题；规则职责与 UI 职责的边界更重要                 |

这些是静态代码审查结论；并不意味着每一种潜在连锁错误都已在现有卡牌中复现。当前 139 项通过的测试是迁移基线，不代表未来效果组合已获得覆盖。
...
| 阶段              | 工作                                                                      | 完成标准                                                                           |
| ----------------- | ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| 0：固化行为       | 记录当前 139 项测试基线，保存关键局面与输入记录；区分既有规则和已确认错误 | 电梯、坍塌、感染、死亡、敌人阶段、联机骰子归属都有可重复验收用例；不靠截图判定规则 |
...
| 3：迁移房间与剧本 | 将电梯/坍塌改为多步流程；狼人、钟声等逐个注册生命周期行为                 | 同一玩法在旧、新路径最终权威状态一致；地图关系、乘员与道具位置保持合法             |
| 4：接通展示与网络 | UI 使用能力/检定模型；结构化手记与动画；服务端投影                        | 预览/结算参数一致；移动先于受击展示；无资格玩家不能响应；隐藏字段不在其网络响应中  |
| 5：淘汰兼容路径   | 迁移剩余内容、删除 _rollCapture/_rollReplay 与重复预览分支                | 效果注册表覆盖全部现有内容；旧存档策略明确；生产入口只留一条新调度路径             |

阶段 1–3 每次只迁移一个封闭流程，默认继续使用稳定路径；测试工具可选择新路径，禁止一局同时对同一动作应用两套引擎。新增功能实验与迁移行为修正使用独立提交。

旧快照可直接映射的字段写版本化迁移器。旧版 diceRequest 的 resumeAction/resumeQueue 无法自动假定等于新步骤位置：未完成旧局先由旧规则版本继续到安全边界，再迁移；无法继续的版本提供原存档导出及明确提示，不能静默重掷或抹掉进度。
