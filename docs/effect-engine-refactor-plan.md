...
2026-09-15。状态：重构实施中。可序列化流程已覆盖交互式事件检定和玩家对抗攻击；Modifier 已接管卡牌检定、治疗、移动、精神减伤、月光力量和伤害上限；Trigger 已接管狼毒/保护时钟与月光回血；Action 注册表已接管神秘电梯、主动坠落和地下室回程。敌人整轮行动、房间发现检定、作祟检定及剧本目标仍有兼容路径，引擎重构尚未完成。
...
| Modifier   | sum/max/unique/replace/cap、来源追踪、卡牌和月光查询、剧本运行时修正 | 地图拓扑、人物轨道和物品实例的有效规则视图       |
...
1. 补齐 `effects` 基础操作与 Before/AfterDamage 时机，用一件“受伤后反击一次”的测试物品验证来源、限次、死亡中断和保存恢复。
2. 将物品改为 definition + instance，迁移使用次数、转移与临时能力；卡牌目录保持只读基础定义。
3. 建立房间和人物轨道的有效规则视图，再迁移神秘电梯、坍塌房间和封窗，统一预览、合法性检查与执行参数。
4. 将敌人逐个移动/攻击迁入 Workflow，输出结构化移动、投骰和伤害事件；随后接服务端玩家视图投影与手机端抽屉布局。
...
首个交付切片应是：同一张测试事件卡完成“首次检定 → 成功后再次检定 → 承受伤害 → 触发一次反应”，且能在每个等待点保存恢复。这比先迁移全部卡牌更能证明架构可用。
...
这些是静态代码审查结论；并不意味着每一种潜在连锁错误都已在现有卡牌中复现。当前 134 项通过的测试是迁移基线，不代表未来效果组合已获得覆盖。
...
| 阶段              | 工作                                                                      | 完成标准                                                                           |
| ----------------- | ------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| 0：固化行为       | 记录当前 134 项测试基线，保存关键局面与输入记录；区分既有规则和已确认错误 | 电梯、坍塌、感染、死亡、敌人阶段、联机骰子归属都有可重复验收用例；不靠截图判定规则 |
