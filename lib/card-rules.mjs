import { itemUseDefinition } from './content/item-abilities.mjs';
...
export function resolveCard(game, type, id, heroId) {
  ...
  if (!base) return null;
  const card = {
    // Discovering ordinary events and items leaves any remaining movement intact.
    // Omens are the default interruption; individual cards and scenario rules may override it.
    stopsMovement: type === 'omen',
    useAmount: 2,
...
function validPatch(patch) {
  ...
  return Object.entries(patch).every(([key, value]) => {
    ...
    if (key === 'use')
      return (
        value === null ||
        (typeof value === 'string' && !!itemUseDefinition(value))
      );
