...
function validPatch(patch) {
  ...
  return Object.entries(patch).every(([key, value]) => {
    ...
    if (['cureBonus', 'threshold', 'useAmount', 'delta'].includes(key))
...
function heldCards(game, hero) {
  return ['item', 'omen'].flatMap((type) =>
    hero[type === 'item' ? 'items' : 'omens'].map((id, index) => ({
      type,
      id,
      index,
      card: resolveCard(game, type, id, hero.id),
    })),
  );
}
...
export function cardModifierContributions(game, query, context = {}) {
      ...
      .flatMap((id) => {
        ...
        return owner
          ? heldCards(game, owner)
              ...
              .map(({ type, id: cardId, index, card }) => ({
                id: `card:${type}:${cardId}:cure`,
                sourceId: `hero:${owner.id}:${type}:${index}:${cardId}`,
  ...
  if (!hero) return [];
  return heldCards(game, hero).flatMap(({ type, id, index, card }) => {
    let applies = false,
      group = card?.bonus;
    ...
    return [
      {
        ...
        sourceId: `hero:${hero.id}:${type}:${index}:${id}`,
        sourceLabel: card.title,
