...
import { bloodmoonActions } from './bloodmoon-mechanics.mjs';
...
const ROOM_TARGET_ACTIONS = {
  moonSeal: {
    ...
    checkSpec: {
      ...
      checkKind: 'ritual',
      includeAll: false,
      threshold: WOLF_RULES.ritualThreshold,
    },
    outcomes: ({ room }) =>
      thresholdRows(
  ...
  seal: {
    ...
    checkSpec: ({ game }) => ({
      ...
      checkKind: 'ritual',
      includeAll: true,
      threshold: 3 + Math.floor((game.count - 3) / 2),
    }),
    outcomes: ({ game }) =>
      thresholdRows(
...
export function contentActions(game, context) {
  ...
  actions.push(...bloodmoonActions(game, context.hero, context.room));
    ...
    if (target && !context.room.done && target.available({ game, ...context }))
      actions.push({
        ...
        ...(target.checkSpec
          ? {
              checkSpec: {
                ...(typeof target.checkSpec === 'function'
                  ? target.checkSpec({ game, room: context.room })
                  : target.checkSpec),
  ...
  for (const target of game.heroes.filter(
    ...
    actions.push({
      ...
      checkSpec: {
        ...
        bonus,
        threshold: WOLF_RULES.cureThreshold,
      },
