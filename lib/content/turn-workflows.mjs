...
export const ROOM_ENTRY_WORKFLOW = 'room.entry';
...
export const TIMING_WORKFLOW = 'trigger.timing';
...
export const TURN_WORKFLOW_DEFINITIONS = {
  ...
  [HERO_MOVE_WORKFLOW]: {
    ...
    steps: [
      ...
      { op: 'effect', handler: 'room.enter.card' },
      { op: 'complete' },
    ],
  },
  [ROOM_ENTRY_WORKFLOW]: {
    id: ROOM_ENTRY_WORKFLOW,
    version: 1,
    steps: [
      { op: 'effect', handler: 'room.enter.before' },
      { op: 'effect', handler: 'room.enter.resolve' },
  ...
  [TIMING_WORKFLOW]: {
    ...
    version: 1,
    steps: [
      { op: 'effect', handler: 'trigger.timing.run' },
      { op: 'complete' },
    ],
  },
};
...
export function createTimingWorkflow({ flowId, event, noticeUid }) {
  return createWorkflow({
    flowId,
    definitionId: TIMING_WORKFLOW,
    locals: {
  ...
  return createWorkflow({
    ...
    locals: {
      move: {
        ...
        moveCost,
        enterKind,
        ...(collapseDice ? { collapseDice: [...collapseDice] } : {}),
      },
    },
  });
}

export function createRoomEntryWorkflow({
  flowId,
  heroId,
  fromRoomId,
  toRoomId,
  ...
  return createWorkflow({
    flowId,
    definitionId: ROOM_ENTRY_WORKFLOW,
    locals: {
      move: {
        heroId,
        fromRoomId,
