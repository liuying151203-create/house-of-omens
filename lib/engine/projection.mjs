...
export function projectGameForPlayer(game, room, playerId) {
  ...
  for (const tile of projected.rooms || []) {
    if (!tile.traps) continue;
    tile.traps = tile.traps.filter((trap) => factions.has(trap.faction));
    // 空字段也会泄露曾在哪个房间布置过敌对陷阱。
    if (!tile.traps.length) delete tile.traps;
  }
