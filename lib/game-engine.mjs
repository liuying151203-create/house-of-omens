...
import { fireTiming } from './triggers.mjs';
import { executeAction } from './engine/actions.mjs';
...
import { itemActionViews, selectItemAction } from './content/item-actions.mjs';
import { contentActions } from './content/actions.mjs';
import {
  advanceWorkflow,
  supplyRolls,
  workflowRollTotal,
...
function damage(s, h, kind, amount, cause = {}) {
  ...
  const original = Math.max(0, Math.trunc(amount)),
    ...
    before = fireRuleTiming(s, draft, cause.report, cause.triggerUsage);
  amount = Math.max(0, Math.trunc(draft.amount));
  if (kind === 'mental')
    amount = Math.max(
      0,
      amount - modifierValue(s, 'damage.mental.prevent', { heroId: h.id }),
    );
  if (amount > 0)
    enqueue(s, {
      kind: 'damage',
      ...
      triggerUsage: before.usage,
      beforeTriggers: before.executed,
    });
  else if (original > 0) {
    log(
      s,
      `${h.name}的${original}点${kind === 'physical' ? '肉体' : '精神'}伤害已被抵消，属性未下降。`,
    );
    fireAfterDamage(
      s,
      {
...
function fireAfterDamage(s, pendingDamage, hero, allocation) {
  return fireRuleTiming(
    s,
    {
      when: 'AfterDamage',
      rootActionId:
        pendingDamage.rootActionId ||
        pendingDamage.causeId ||
        `damage:${pendingDamage.uid || s.serial}`,
      causeId: pendingDamage.causeId,
      heroId: hero.id,
...
function workflowDefinitions() {
  return { ...CARD_WORKFLOW_DEFINITIONS, ...COMBAT_WORKFLOW_DEFINITIONS };
}
...
function queueWorkflowRequest(s, next, extra = {}) {
  s.queue.unshift({
    ...
    ...next.request,
    ...extra,
    rolls: next.request.rolls.map((r, index) => ({
      ...r,
      id: 'side-' + index,
      dice: null,
    })),
...
function startEventWorkflow(state, cardMessage) {
  ...
  const flow = createEventCheckWorkflow({
      ...
      heroId: h.id,
      card: { ...card, traitLabel: TRAITS[card.trait] },
      diceCount: eventDiceCount(s, h, card),
      rollLabel: h.name + ' · ' + TRAITS[card.trait],
...
function startHeroAttackWorkflow(state, action) {
  const s = structuredClone(state),
    ...
    legal = actions(s);
  if (!hero || !enemy || !legal.attack.includes(enemy.id)) return s;
  const attackCount = modifierValue(
      s,
      'check.dice',
      { heroId: hero.id, checkKind: 'attack', trait: 'might' },
      traitValue(hero, 'might'),
    ),
...
function fallIntoRoom(s, h, source, destination, discovered = false) {
  source.collapseLanding = destination.id;
  destination.tokens ||= [];
...
function operateElevator(s, h, r) {
  h.moves -= 1;
  const dice = roll(s, 2, { heroId: h.id, label: h.name + ' · 神秘电梯' });
...
const RULE_ACTION_HANDLERS = {
  ...
  'room.basement.findReturn': (s, _command, _action, context) => {
    ...
    s.links.push(['basement', 'foyer']);
    log(s, context.hero.name + '在地下室平台找到了回程暗梯，通向一楼门厅。');
  },
  'item.use': (s, _command, action, context) => {
    const result = executeEffects(s, action.effects, RULE_EFFECT_HANDLERS, {
      event: {
    ...
    const instance = itemInstances(context.hero).find(
      (entry) => entry.instanceId === action.instanceId,
    );
    if (!instance) return;
    markItemInstanceUsed(context.hero, instance);
    if (action.consume) removeItemInstance(context.hero, instance.instanceId);
    const card = cardDefinition(s, 'item', action.cardId, context.hero.id),
      changes = result.executed
    ...
    announce(s, '使用' + card.title, resultText, 'itemUse', {
      ...
      effects: result.executed,
    });
    log(s, `${context.hero.name}使用「${card.title}」：${resultText}`);
  },
};
function enterRoom(s, h, r) {
  if (r.special === 'collapse' && !r.collapseChecked) {
    r.collapseChecked = true;
    const dice = roll(s, traitValue(h, 'speed'), {
      heroId: h.id,
    ...
    if (total < 5) {
      ...
      announce(
        ...
        'check',
        { dice, total, threshold: 5, success: true },
      );
  }
  if (
    r.special === 'stairsDown' &&
...
function resolveCard(s, p) {
  ...
  if (p.cardType === 'event') {
    ...
    announce(
      ...
      'cardResult',
      { cardType: 'item', itemInstanceId: instance.instanceId },
    );
...
function finishArrival(s) {
  ...
  if (!h.dead && destination) {
    ...
    if (destination.icon) drawCard(s, destination.icon, h);
  }
}
function advance(s) {
  ...
  if (p.kind === 'roomFall') {
    ...
    if (p.roomId && p.discovered)
      enqueue(s, { kind: 'roomArrival', heroId: h.id, roomId: p.roomId });
...
function applyAction(state, a) {
  ...
  if (p) {
    ...
    if (p.kind === 'damage') {
      if (a.type === 'allocateDamage') {
        ...
        for (const key of keys) {
          if (a.allocation[key] > 0) {
            const before = h.stats[key];
            changes.push(changeTrait(s, h, key, -a.allocation[key]));
            p.actualApplied = (p.actualApplied || 0) + before - h.stats[key];
          }
        }
        if (h.dead)
        ...
        announce(
          ...
          'damageResult',
          {
            heroId: h.id,
            changes,
            rawAmount: p.rawAmount ?? p.total + (p.prevented || 0),
            preventedAmount: p.prevented || 0,
            actualAmount: p.actualApplied || 0,
            sourceKind: p.sourceKind,
            sourceEnemyId: p.sourceEnemyId,
          },
        );
        const damageResult = s.queue.at(-1);
        log(
          s,
          `${h.name}结算${p.remaining}点${p.damageType === 'physical' ? '肉体' : '精神'}伤害${p.prevented ? '（另抵消' + p.prevented + '点）' : ''}：${changes.join('；')}${h.dead ? '；人物已死亡' : ''}。`,
        );
        const after = fireAfterDamage(s, p, h, a.allocation);
        damageResult.triggers = [
          ...(p.beforeTriggers || []),
          ...after.executed,
        ];
        check(s);
      ...
      const h = s.heroes[p.heroId];
      const before = h.stats[a.trait];
      const change = changeTrait(s, h, a.trait, -1);
      p.actualApplied = (p.actualApplied || 0) + before - h.stats[a.trait];
      p.remaining--;
      p.allocation.push(TRAITS[a.trait]);
      log(
        s,
        `${h.name}结算1点${p.damageType === 'physical' ? '肉体' : '精神'}伤害：${change}${h.dead ? '；人物已死亡' : ''}。`,
      );
      const complete = p.remaining <= 0 || h.dead;
      if (complete) s.queue.shift();
      if (h.dead)
        s.queue = s.queue.filter(
          (entry) => entry.kind !== 'damage' || entry.heroId !== h.id,
        );
      if (complete) fireAfterDamage(s, p, h, p.allocation);
      check(s);
      finishArrival(s);
      return s;
    }
  ...
  if (s.scenario === 'werewolf' && s.phase === 'haunt' && !h.interacted) {
    ...
    if (a.type === 'cure') {
      ...
      infection.attempts++;
      const bonus = infection.attempts - 1 + cureBonus(s, h, target);
  ...
  if (
    executeAction(s, a, legal.abilities, RULE_ACTION_HANDLERS, {
      hero: h,
      room: r,
  ...
  if (a.type === 'useItem') {
    const itemAction = selectItemAction(s, h, a, TRAITS);
    if (!itemAction) return s;
    executeAction(
      s,
      { ...a, type: itemAction.id },
      [itemAction],
  ...
  if (a.type === 'attack' && legal.attack.includes(a.id)) {
    const e = s.enemies.find((e) => e.id === a.id),
      attack = roll(
        ...
        modifierValue(
          s,
          'check.dice',
          { heroId: h.id, checkKind: 'attack', trait: 'might' },
          traitValue(h, 'might'),
        ),
    ...
    resolveHeroAttack(
      ...
      defense,
      s.silver,
      modifierValue(s, 'combat.damage.cap', { enemyId: e.id }, 3),
      {
        rootActionId: `attack:${s.round}:${h.id}:${e.id}:${s.serial + 1}`,
      },
    );
    return s;
  }
    ...
    if (r.target === 'seal') {
      const dice = roll(
          ...
          modifierValue(
            s,
            'check.dice',
            { heroId: h.id, checkKind: 'ritual', trait: 'knowledge' },
            traitValue(h, 'knowledge'),
          ),
...
function applyInteractiveAction(state, a) {
  ...
  if (current?.kind === 'diceRequest') {
    ...
    if (
      ...
      if (p.workflow) {
        ...
        const result = s.queue.find((entry) =>
          ['cardResult', 'combat'].includes(entry.kind),
        );
        receipt.text = result?.text || s.result?.reason || '';
        if (s.queue[0]) {
          s.queue[0].rollReceipt = receipt;
          if (s.queue[0].dice) s.queue[0].dicePresented = true;
  ...
  if (a.type === 'attack') return startHeroAttackWorkflow(state, a);
  ...
  s.queue = [
    {
      ...
      outcomes: rollOutcomes(state, a, current),
...
const RULE_TRIGGER_HANDLERS = {
  'status.remove': (s, _event, params) => {
    ...
    return hero.statuses.length !== before;
  },
  'status.adjust': (s, _event, params) => {
    const hero = s.heroes[params.heroId],
...
const RULE_EFFECT_HANDLERS = {
  ...
  'hero.restoreTrait': (s, params) => {
    ...
    if (
      ...
      params.amount <= 0 ||
      hero.stats[params.trait] >= hero.start[params.trait]
    )
      return false;
    const delta = Math.min(
      params.amount,
      hero.start[params.trait] - hero.stats[params.trait],
    );
    return {
      heroId: hero.id,
  ...
  'status.add': (s, params) => {
    ...
    const status = {
      ...
      instanceId: params.status.instanceId || `status-${++s.serial}`,
    };
    hero.statuses ||= [];
  ...
  'status.remove': (s, params) => {
    ...
    return hero.statuses.length === before
      ? false
      : { heroId: hero.id, removed: before - hero.statuses.length };
  },
};

function fireRuleTiming(s, event, report, usage = []) {
  return fireTiming(s, event, RULE_TRIGGER_HANDLERS, {
    report,
    usage,
