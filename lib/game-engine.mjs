...
function damage(s, h, kind, amount) {
  ...
  if (amount > 0)
    ...
    log(s, `${h.name}的${original}点精神伤害已被抵消，属性未下降。`);
  return amount;
}
...
function resolveHeroAttack(s, h, e, attack, defense, attackBonus, damageCap) {
  h.attacked = true;
...
function fireRuleTiming(s, event, report) {
  return fireTiming(s, event, RULE_TRIGGER_HANDLERS, { report });
}
