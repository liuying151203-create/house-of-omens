...
function applyAction(state, a) {
  ...
  if (a.type === 'useItem') {
    const instance = itemInstances(h).find((entry) =>
      a.instanceId
        ? entry.instanceId === a.instanceId
        : entry.definitionId === a.id && !itemInstanceUsed(h, entry),
    );
    if (!instance || itemInstanceUsed(h, instance)) return s;
    const c = cardDefinition(s, 'item', instance.definitionId, h.id);
    if (!c.use) return s;
    let change = '';
    if (c.use === 'movement') {
      if (h.stopped) {
    ...
    markItemInstanceUsed(h, instance);
    if (c.consumable) removeItemInstance(h, instance.instanceId);
