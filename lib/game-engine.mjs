...
function operateElevator(s, h, r) {
  ...
  const destinations = roomDestinations(s, r, floors, r.id);
  const text =
    '电梯投出 ' +
    total +
    ' 点：' +
...
export function actions(s) {
  ...
  return {
    ...
    elevator: r.special === 'elevator' && !h.elevatorUsed && !h.stopped,
    fall: r.special === 'collapse' && r.collapseChecked && !h.stopped,
    // Basement escape adaptation: a permanent stair can be discovered from below too.
    returnStairs:
      r.id === 'basement' && !s.basementUnlocked && canMove && h.moves >= cost,
    moveCost: cost,
  };
}
function endRound(s) {
...
function applyAction(state, a) {
  ...
  if (a.type === 'useElevator' && legal.elevator) {
    operateElevator(s, h, r);
    return s;
  }
  if (a.type === 'jumpDown' && legal.fall) {
    startFall(s, h, r);
    return s;
  }
  if (a.type === 'findReturnStairs' && legal.returnStairs) {
    h.moves -= legal.moveCost;
    s.basementUnlocked = true;
    s.links.push(['basement', 'foyer']);
    log(s, h.name + '在地下室平台找到了回程暗梯，通向一楼门厅。');
    return s;
  }
  if (a.type === 'endHero') {
    h.ended = true;
