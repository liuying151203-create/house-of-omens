...
import { canCleanseStatus } from './status-rules.mjs';
import {
  resolveGameSet,
  validateGameSet,
  gameSetOmenCount,
...
import {
  BLOODMOON_WORKFLOWS,
  createBloodmoonFlow,
  createStatusGainFlow,
  prepareBloodmoon,
  chooseBloodmoon,
  rollBloodmoon,
  performBloodmoonAction,
  bloodmoonTriggers,
  hostileTrap,
} from './content/bloodmoon-mechanics.mjs';
import {
  CLEANSE_WORKFLOW_DEFINITIONS,
  createCleanseWorkflow,
  prepareCleanseChoice,
  selectCleanseStatus,
  selectedCleanseStatuses,
} from './content/cleanse-workflow.mjs';
...
function createGameState(scenario, seed = Date.now(), count = 3, options = {}) {
  ...
  const gameSet = resolveGameSet(options.gameSet);
  // 旧调用保持原状态形状和洗牌顺序；显式选择时冻结本局配置。
  if (options.gameSet != null) s.gameSet = gameSet;
  for (const kind of ['rooms', 'event', 'item', 'omen'])
    s.decks[kind] = shuffle(s, gameSet.decks[kind]);
...
export function validSave(s) {
  try {
    if (s?.gameSet !== undefined) validateGameSet(s.gameSet);
    return (
...
function continueWorkflow(s, flow) {
  return advanceWorkflow(flow, {
    ...
    handlers: {
      'bloodmoon.prepare': (_params, activeFlow) =>
        prepareBloodmoon(s, activeFlow, bloodmoonContext(s)),
      'bloodmoon.choose': (_params, activeFlow) =>
        chooseBloodmoon(s, activeFlow, bloodmoonContext(s)),
      'bloodmoon.roll': (_params, activeFlow) =>
        rollBloodmoon(s, activeFlow, bloodmoonContext(s)),
      'bloodmoon.finish': (_params, activeFlow) =>
        finishBloodmoonWorkflow(s, activeFlow),
      'status.gain.before': (_params, activeFlow) =>
        runWorkflowTiming(s, activeFlow, activeFlow.locals.draft),
      'status.gain.commit': (_params, activeFlow) =>
        commitStatusGain(s, activeFlow.locals.draft),
      'item.cleanse.prepare': (_params, activeFlow) =>
        prepareCleanseChoice(activeFlow),
      'item.cleanse.select': (_params, activeFlow) =>
        selectCleanseStatus(activeFlow),
      'item.cleanse.commit': (_params, activeFlow) =>
        commitCleanseWorkflow(s, activeFlow),
...
function attachReactionResume(s, paused, resume, receipt) {
  ...
  let targetFlow = request?.workflow;
  while (
    paused?.flowId &&
    targetFlow?.flowId !== paused.flowId &&
    targetFlow?.locals?.reaction?.resume?.flow
  )
    targetFlow = targetFlow.locals.reaction.resume.flow;
  const reaction = targetFlow?.locals?.reaction;
...
function resolveHauntRollWorkflow(s, flow) {
  ...
  const dice = flow.locals.rolls.haunt[0],
    ...
    triggers = total >= 5 || s.omens >= gameSetOmenCount(s);
  announce(
    s,
...
function prepareEnemyWorkflow(s, flow) {
  ...
  while (turn.enemyIndex < turn.enemyIds.length) {
    ...
    const path = pathTo(s, enemy.pos, target.pos, { monster: true }),
      hazard = resolveEnemyHazards(
        s,
        enemy,
        path.length ? path.slice(0, enemy.speed + 1) : [enemy.pos],
        flow,
      );
    if (hazard.paused) return { suspend: true, nextStep: flow.step };
    if (!s.enemies.some((entry) => entry.id === enemy.id)) return;
    const route = hazard.route,
...
function resolveEnemyWorkflowAttack(s, flow) {
  ...
  commitEnemyMovement(s, enemy, current, turn.report);
  for (
    let index = current.resolvedVictims || 0;
    index < current.victimIds.length;
    index++
  ) {
    const heroId = current.victimIds[index];
    current.resolvedVictims = index + 1;
    ...
    const infection =
      ...
      requestWerewolfInfection(s, target, {
        rootActionId: `enemy-turn:${s.round}:${enemy.id}`,
    ...
    if (infection?.paused) {
      attachReactionResume(s, infection.paused, {
        kind: 'workflow',
        flow,
        event: { when: 'EnemyAttackStatus', enemyId: enemy.id },
        remainingTriggers: [],
        usage: [],
      });
      return { suspend: true, nextStep: flow.step };
    }
    if (infection?.executed?.length)
      turn.report.push(
...
function bloodmoonContext(s) {
  return {
    log: (text, visibility) => log(s, text, visibility),
    traitDice: (hero, trait) =>
      modifierValue(
        s,
        'check.dice',
        { heroId: hero.id, trait, checkKind: 'bloodmoon' },
        traitValue(hero, trait, s),
      ),
    doorNeighbors: (roomId) => {
      const room = roomAt(s, roomId);
      return doorsOf(room).flatMap((dir) => {
        const adjacent = tileAt(
          s,
          room.floor,
          room.x + DIRS[dir].dx,
          room.y + DIRS[dir].dy,
        );
        return adjacent && doorsOf(adjacent).includes((dir + 2) % 4)
          ? [adjacent.id]
          : [];
      });
    },
    start: (params) => startBloodmoonEffect(s, createBloodmoonFlow(s, params)),
    hurtEnemy: (enemyId, amount) =>
      damageEnvironmentalEnemy(s, enemyId, amount),
    reroll: (locals, candidate) =>
      RULE_EFFECT_HANDLERS['check.reroll'](
        s,
        { rollIndex: candidate.rollIndex, diceIndices: [candidate.index] },
        {
          event: locals.reaction.resume?.event || locals.reaction.event,
          resume: locals.reaction.resume,
        },
      ),
  };
}

function damageEnvironmentalEnemy(s, enemyId, amount) {
  const enemy = s.enemies.find((entry) => entry.id === enemyId);
  if (!enemy) return;
  enemy.movementStoppedRound = s.round;
  enemy.hp = Math.max(0, enemy.hp - amount);
  log(s, `${enemy.name}受到${amount}点环境伤害，停止本回合移动。`);
  recordEvent(s, 'EnemyDamaged', { enemyId, amount, sourceKind: 'room' });
  defeatEnemy(s, enemy);
  check(s);
}

// 仅含新环境效果的路径走逐房处理；旧剧本保留原随机数与移动结算顺序。
function resolveEnemyHazards(s, enemy, proposedRoute, flow) {
  const turn = flow?.locals.enemyTurn;
  const hasHazards = proposedRoute.some(
    (id) =>
      hostileTrap(roomAt(s, id), enemy.faction || 'wolves') ||
      s.bloodmoon?.steam?.some(
        (steam) => steam.active && steam.roomIds.includes(id),
      ),
  );
  if (!turn?.hazardRoute && !hasHazards) return { route: proposedRoute };
  const state = turn?.hazardRoute || {
    route: proposedRoute,
    index: 0,
    visited: [proposedRoute[0]],
  };
  if (turn) turn.hazardRoute = state;
  while (state.index < state.route.length) {
    const roomId = state.route[state.index];
    if (!s.enemies.some((entry) => entry.id === enemy.id)) break;
    if (state.index > 0 && enemy.pos !== roomId) {
      if (enemy.movementStoppedRound === s.round) break;
      enemy.pos = roomId;
      const explorer = enemyExplorer(s, enemy);
      if (explorer) explorer.pos = roomId;
      state.visited.push(roomId);
      const room = roomAt(s, roomId),
        trap = hostileTrap(room, enemy.faction || 'wolves');
      if (trap) {
        room.traps = room.traps.filter((entry) => entry.id !== trap.id);
        log(s, `${enemy.name}触发捕兽夹。`);
        damageEnvironmentalEnemy(s, enemy.id, 2);
        if (!s.enemies.some((entry) => entry.id === enemy.id)) break;
      }
    }
    const key = `enemy:${enemy.id}:${roomId}`,
      steam = s.bloodmoon?.steam?.find(
        (entry) =>
          entry.active &&
          entry.roomIds.includes(roomId) &&
          !entry.checked.includes(key),
      );
    if (steam) {
      steam.checked.push(key);
      if (flow) {
        const paused = startBloodmoonEffect(
          s,
          createBloodmoonFlow(s, {
            kind: 'enemySteam',
            heroId: enemy.heroId ?? s.active,
            enemyId: enemy.id,
            roomId,
          }),
        );
        if (paused?.pause) {
          attachReactionResume(s, paused, {
            kind: 'workflow',
            flow,
            event: { when: 'EnemyRoomEnter', enemyId: enemy.id },
            remainingTriggers: [],
            usage: [],
          });
          return { paused: true };
        ...
        const dice = roll(s, Math.max(1, Math.min(16, enemy.speed)), {
          heroId: enemy.heroId,
          label: enemy.name + ' · 避开蒸汽',
        });
        if (dice.reduce((sum, face) => sum + face, 0) < 4)
          damageEnvironmentalEnemy(s, enemy.id, 1);
      }
      continue;
    }
    // 停止后仍须结算已进入房间的全部效果，只禁止继续走向下一间。
    if (enemy.movementStoppedRound === s.round) break;
    state.index++;
  }
  if (turn) delete turn.hazardRoute;
  return { route: state.visited };
}

function startBloodmoonEffect(s, flow) {
  const next = continueWorkflow(s, flow);
  const request =
    next.status === 'waiting'
      ? queueWorkflowRequest(s, next)
      : next.status === 'suspended'
        ? s.queue.find(
            (entry) =>
              entry.kind === 'contentResume' &&
              entry.workflow?.flowId === flow.flowId,
          ) || pending(s)
        : null;
  return request
    ? { pause: true, requestId: request.uid, flowId: flow.flowId }
    : true;
}

function finishBloodmoonWorkflow(s, flow) {
  const l = flow.locals;
  if (l.damage && !l.damageCommitted) {
    l.damageCommitted = true;
    damage(s, s.heroes[l.heroId], 'physical', l.damage, {
      rootActionId: flow.flowId,
      causeId: flow.flowId,
      sourceKind: 'room',
      sourceRoomId: l.roomId,
      causeTag: l.kind,
    });
    enqueue(s, {
      kind: 'contentResume',
  ...
  const reaction = l.reaction,
    context = {
      event: reaction.resume?.event || reaction.event,
      resume: reaction.resume,
    };
  const continuation = executeRuleEffects(
    s,
  ...
  if (continuation.paused)
    attachReactionResume(s, continuation.paused, reaction.resume || {});
  else resumeReactionContinuation(s, reaction.resume);
}

function commitStatusGain(s, event) {
  const hero = s.heroes[event.heroId];
  if (event.cancelled || !hero || hero.dead) return false;
  const existing = hero.statuses?.find(
    (status) => status.id === event.status.id,
  );
  if (existing && Number.isInteger(event.status.level)) {
    if ((existing.level || 0) >= event.status.level) return false;
    const instanceId = existing.instanceId,
      before = existing.level;
    Object.assign(existing, structuredClone(event.status), { instanceId });
    recordEvent(
      s,
      'StatusChanged',
      {
    ...
    RULE_EFFECT_HANDLERS['status.add'](s, {
      heroId: hero.id,
      status: event.status,
    });
  return true;
...
const RULE_ACTION_HANDLERS = {
  'item.placeTrap': (s, _command, action, { hero, room }) =>
    performBloodmoonAction(
      s,
      { kind: 'trap', instanceId: action.instanceId },
      hero,
      room,
      bloodmoonContext(s),
    ),
  'bloodmoon.action': (s, _command, action, { hero, room }) =>
    performBloodmoonAction(s, action, hero, room, bloodmoonContext(s)),
  ...
  'item.cleanse': (s, _command, action, context) => {
    const target = s.heroes[action.targetHeroId];
    if (
      !target ||
      target.dead ||
      ...
      return;
    const flow = createCleanseWorkflow(
      `cleanse:${++s.serial}`,
      context.hero,
      target,
      action,
    );
    queueWorkflowRequest(s, continueWorkflow(s, flow));
  },
...
function commitCleanseWorkflow(s, flow) {
  const hero = s.heroes[flow.locals.heroId];
  const action = itemActionViews(s, hero, TRAITS).find(
    (entry) => entry.id === flow.locals.action.id && entry.available,
  );
  if (!action) return;
  const selected = selectedCleanseStatuses(flow);
  RULE_ACTION_HANDLERS['item.use'](
    s,
    {},
    {
      ...action,
      effects: [
        {
          op: 'status.cleanse',
          params: { heroId: action.targetHeroId, selections: selected },
        },
      ],
      resultText: `${s.heroes[action.targetHeroId].name}清除了${selected.map((entry) => entry.label).join('、')}。`,
    },
    { hero },
  );
}
...
function advance(s) {
  ...
  if (!p) return;
  if (p.kind === 'contentResume') {
    const next = continueWorkflow(s, p.workflow);
    if (next.status === 'waiting') queueWorkflowRequest(s, next);
    return;
  }
  if (p.kind === 'roomFall') {
    ...
    announce(
      s,
      total >= 5 || s.omens >= gameSetOmenCount(s)
        ? '有东西醒来了'
        : '宅邸暂时沉寂',
      total >= 5 || s.omens >= gameSetOmenCount(s)
        ? total >= 5
    ...
    log(
      s,
      `作祟检定 ${total} / 5：${total >= 5 || s.omens >= gameSetOmenCount(s) ? '即将进入作祟' : '尚未触发'}。`,
    );
...
function endRound(s) {
  ...
  if (s.phase === 'haunt') {
    ...
    for (const e of s.enemies) {
      ...
      const route = resolveEnemyHazards(
        s,
        e,
        path.length ? path.slice(0, e.speed + 1) : [e.pos],
      ).route;
      if (s.phase === 'over') return;
      if (!s.enemies.some((entry) => entry.id === e.id)) continue;
      if (route.length > 1) {
        const fromRoomId = route[0];
        e.pos = route.at(-1);
        const explorer = enemyExplorer(s, e);
...
function runStandaloneTurnStartTiming(s, flow) {
  ...
  if (
    !locals.statusExpiredEvents &&
    !s.heroes[locals.lifecycleEvent.heroId]?.statuses?.some(
      (status) => status.id === 'trait-boost' && status.expiresRound <= s.round,
    )
  )
    return runWorkflowTiming(s, flow, locals.lifecycleEvent);
  // 到期反应可以暂停；恢复时沿用同一事件队列，不能重新扣减属性。
  locals.statusExpiredEvents ||= [];
  locals.statusExpiredIndex ||= 0;
  if (!locals.startTimingDone) {
    const event = locals.lifecycleEvent,
      triggered = fireRuleTiming(
        s,
        event,
        undefined,
        ...
        {
          allowPause: true,
          deferredEvents: locals.statusExpiredEvents,
        },
      );
    appendWorkflowReceipts(s, flow, event, triggered);
    locals.startTimingDone = true;
    if (triggered.paused) {
      attachReactionResume(s, triggered.paused, {
        kind: 'workflow',
        flow,
        ...
        remainingTriggers: triggered.remainingTriggers,
        usage: triggered.usage,
        deferredEventPath: 'statusExpiredEvents',
      });
      return { suspend: true, nextStep: flow.step };
    }
  }
  while (locals.statusExpiredIndex < locals.statusExpiredEvents.length) {
    const event = locals.statusExpiredEvents[locals.statusExpiredIndex++],
      triggered = fireRuleTiming(
        s,
        event,
        undefined,
...
function applyInteractiveAction(state, a) {
  ...
  if (
    (state.executionMode === GAME_EXECUTION_MODES.simulation &&
      !BLOODMOON_WORKFLOWS[pending(state)?.workflow?.definitionId]) ||
    state.phase === 'over'
  )
...
const RULE_TRIGGER_HANDLERS = {
  'status.expireTraitBoost': (s, event, params, context) => {
    const hero = s.heroes[params.heroId],
      status = hero?.statuses?.find(
        (entry) =>
          entry.id === 'trait-boost' && entry.instanceId === params.instanceId,
      );
    if (!hero || hero.dead || !status || status.expiresRound > s.round)
      return false;
    const beforeMovement = movement(hero, s);
    // 按到期时的位置回退，不能恢复使用前快照，否则会抹掉期间的伤害。
    if (TRAIT_KEYS.includes(status.trait) && hero.stats[status.trait] > 1)
      changeTrait(s, hero, status.trait, -1, {
        ...event,
        sourceKind: 'item',
        sourceCardId: status.sourceCardId,
        visibility: heroVisibility(s, hero),
        triggerUsage: context.usage,
      });
    // 每轮开始已预分配所有角色的移动点；到期发生在持有者开回合时。
    if (status.trait === 'speed' && !hero.stopped)
      hero.moves = Math.max(0, hero.moves + movement(hero, s) - beforeMovement);
    return RULE_TRIGGER_HANDLERS['status.remove'](
      s,
      event,
      { ...params, statusId: status.id, expired: true },
      context,
    );
  },
  'status.remove': (s, _event, params, context) => {
    ...
    const hero = s.heroes[params.heroId],
      removed = hero?.statuses?.filter(matches) || [];
    if (!hero) return false;
    hero.statuses = (hero.statuses || []).filter((status) => !matches(status));
...
const RULE_EFFECT_HANDLERS = {
  'bloodmoon.flow': (s, params, context) => {
    if (context.atomicDepth) throw new Error('交互式内容效果不能放入原子批次');
    if (context.event?.cancelled && params.kind === 'badge') return false;
    return startBloodmoonEffect(s, createBloodmoonFlow(s, params, context));
  },
  'status.gain': (s, params, context) => {
    if (
      !s.heroes[params.heroId] ||
      s.heroes[params.heroId].dead ||
      !params.status?.id
    )
      return false;
    const event = {
    ...
    if (
      !bloodmoonTriggers(s, event).length &&
      !s.ruleTriggers?.some((trigger) => trigger.when === 'BeforeStatusGain')
    )
      return commitStatusGain(s, { ...event, cancelled: false });
    if (context.atomicDepth)
      throw new Error('状态获取反应不能放入原子批次，请将支付与状态获取分开');
    return startBloodmoonEffect(s, createStatusGainFlow(s, params, context));
  },
  'status.cleanse': (s, params) => {
    const hero = s.heroes[params.heroId],
    ...
    if (
      ...
      !selections.length ||
      selections.length > 2 ||
      new Set(selections.map((entry) => entry.index)).size !==
        selections.length ||
      selections.some((entry) => {
        const status = hero.statuses?.[entry.index];
        return (
          !Number.isInteger(entry.index) ||
          !canCleanseStatus(status) ||
          status.id !== entry.id ||
          status.instanceId !== entry.instanceId
        );
      })
    )
      return false;
    const removed = selections.map((entry) => hero.statuses[entry.index]);
    hero.statuses = hero.statuses.filter(
      (_status, index) => !selections.some((entry) => entry.index === index),
    );
    for (const status of removed)
      recordEvent(
        s,
    ...
    return { heroId: hero.id, removed: removed.map((status) => status.id) };
  },
...
export function applyWerewolfInfection(state, hero, cause = {}) {
  const result = requestWerewolfInfection(state, hero, cause);
  return !!result && result.executed.length === 1 && !result.paused;
}

function requestWerewolfInfection(state, hero, cause = {}) {
  if (
    !hero ||
    hero.dead ||
    hero.traitor ||
