    ...
    for (const e of s.enemies) {
      ...
      if (e.pos === target.pos) {
        ...
        for (const target of victims) {
          const attack = roll(s, wolfMight(s, e), enemyDiceMeta(s, e)),
            defense = roll(
              s,
  ...
  if (s.scenario === 'werewolf' && s.phase === 'haunt' && !h.interacted) {
    ...
    if (a.type === 'cure') {
      ...
      const bonus =
        infection.attempts -
        1 +
        (h.omens.includes('locket') || target.omens.includes('locket') ? 2 : 0);
  ...
  if (a.type === 'interact' && legal.interact) {
    ...
    if (s.scenario === 'werewolf') {
      ...
      if (r.target === 'moonSeal') {
        ...
        const dice = roll(
          s,
          traitValue(h, 'knowledge') + (hasBonus(h, 'ritual') ? 1 : 0),
          {
            heroId: h.id,
            label: h.name + ' · 净化月印',
            bonus: r.attempts - 1,
