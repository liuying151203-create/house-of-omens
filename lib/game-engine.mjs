...
const enemyDiceMeta = (s, e) => {
  const traitor = e.kind === 'keeper' ? s.heroes.find((h) => h.traitor) : null;
...
export function hasBonus(h, bonus) {
  return (
    h.items.some((id) => ITEMS.find((c) => c.id === id)?.bonus === bonus) ||
    h.omens.some((id) => OMENS.find((c) => c.id === id)?.bonus === bonus)
  );
}
export function movement(h) {
  return traitValue(h, 'speed') + (hasBonus(h, 'movement') ? 1 : 0);
}
...
function damage(s, h, kind, amount) {
  if (h.dead) return;
  if (kind === 'mental' && hasBonus(h, 'mentalWard'))
...
export function drawCard(s, type, h) {
  ...
  enqueue(s, {
    ...
    cardId,
    heroId: h.id,
    title: cardList(type).find((c) => c.id === cardId).title,
    text: '',
    stage: 'draw',
  });
  log(
    s,
    `${h.name}抽取了一张${type === 'event' ? '事件' : type === 'item' ? '物品' : '预兆'}卡，本回合停止移动。`,
  );
}
...
function resolveCard(s, p) {
  const h = s.heroes[p.heroId],
    c = cardList(p.cardType).find((c) => c.id === p.cardId);
  if (p.cardType === 'event') {
    const dice = c.trait
        ? roll(
            s,
            traitValue(h, c.trait) +
              (hasBonus(h, 'all') ? 1 : 0) +
              (hasBonus(h, 'event') && ['sanity', 'knowledge'].includes(c.trait)
                ? 1
                : 0),
    ...
    announce(
      ...
      '已放入' +
        h.name +
        '的随身物品。' +
        ITEMS.find((item) => item.id === c.id).effect,
      'cardResult',
      { cardType: 'item' },
    );
    ...
    h.omens.push(c.id);
    s.omens++;
    const change = changeTrait(s, h, c.trait, c.delta);
    ...
    if (s.phase === 'explore' && s.omens * 2 < 5) {
...
function advance(s) {
  ...
  if (p.kind === 'card') resolveCard(s, p);
    ...
    announce(
      s,
      total >= 5 ? '有东西醒来了' : '宅邸暂时沉寂',
      total >= 5
        ? '总点数达到5，作祟即将降临。'
        : '总点数不足5，继续探索。下一张预兆会让检定多投1枚骰子。',
      'hauntResult',
      { dice, total, threshold: 5, triggers: total >= 5 },
    );
    log(
      s,
      `作祟检定 ${total} / 5：${total >= 5 ? '即将进入作祟' : '尚未触发'}。`,
    );
...
function endRound(s) {
  if (s.phase === 'haunt') {
    ...
    for (const e of s.enemies) {
      ...
      if (e.pos === target.pos) {
        const attack = roll(s, e.might, enemyDiceMeta(s, e)),
          defense = roll(
            s,
            traitValue(target, 'might') + (hasBonus(target, 'all') ? 1 : 0),
...
function applyAction(state, a) {
  ...
  if (
    ...
    const c = ITEMS.find((c) => c.id === a.id);
    if (!c.use) return s;
    let change = '';
    if (c.use === 'movement') {
      if (h.stopped) {
      ...
      change = changeTrait(
        ...
        h,
        a.trait,
        Math.min(2, h.start[a.trait] - h.stats[a.trait]),
      );
    }
    h.used.push(c.id);
  ...
  if (a.type === 'attack' && legal.attack.includes(a.id)) {
    ...
    const e = s.enemies.find((e) => e.id === a.id),
      attack = roll(
        s,
        traitValue(h, 'might') +
          (hasBonus(h, 'attack') ? 1 : 0) +
          (hasBonus(h, 'all') ? 1 : 0),
        { heroId: h.id, label: h.name + ' · 力量', bonus: s.silver },
      ),
      defense = roll(s, e.might, enemyDiceMeta(s, e)),
    ...
    announce(
      ...
      delta > 0
        ...
        : delta === 0
          ? '双方僵持'
          : '对方发动了反击',
      `${h.name} ${aTotal} 对 ${e.name} ${dTotal}。${delta > 0 ? '敌人损失' + delta + '点生命。' : delta < 0 ? '你需要分配' + Math.min(3, -delta) + '点肉体伤害。' : '双方均未受伤。'}`,
      'combat',
      {
        dice: attack,
        defenseDice: defense,
...
export function act(state, a) {
  ...
  if (current?.kind === 'diceRequest') {
    ...
    if (a.type === 'advance' && p.rolls.every((r) => r.dice)) {
  ...
  s.queue = [
    {
      ...
      text:
        current?.kind === 'card'
          ? `${TRAITS[EVENTS.find((c) => c.id === current.cardId)?.trait]} · 目标 ${EVENTS.find((c) => c.id === current.cardId)?.threshold}+`
