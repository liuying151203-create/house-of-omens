...
function startReactionWorkflow(s, params, context = {}) {
  const requestedResponders = Array.isArray(params.responders)
    ...
    options = skipOption
      ? params.options
      : [...(params.options || []), { id: 'skip', label: '跳过', effects: [] }];
  if (
    !responders.length ||
  ...
  const flow = createReactionWorkflow({
      ...
      timeoutChoice: skipOption?.id || 'skip',
      source: {
...
function startActionCheckWorkflow(state, command) {
  ...
  if (!hero || !action?.check) return s;
  const checkSpec = action.check,
    trait = checkSpec.trait,
    baseDice = traitValue(hero, trait, s),
    diceCount =
      checkSpec.applyModifiers === false
        ? baseDice
        : modifierValue(
            s,
            'check.dice',
...
function beginInteractiveEnemyTurn(state, action) {
  const s = structuredClone(state),
    activeHero = s.heroes[s.active];
  if (action.type === 'endHero') {
    finishHeroTurn(s, activeHero);
    activeHero.ended = true;
    activeHero.moves = 0;
    const nextHero = living(s).find((hero) => !hero.ended);
    if (nextHero) {
      s.active = nextHero.id;
      s.viewFloor = roomAt(s, nextHero.pos).floor;
      startHeroTurn(s, nextHero);
      log(s, activeHero.name + '结束行动，轮到' + nextHero.name + '。');
      return s;
    }
  } else {
  ...
  const flow = createEnemyTurnWorkflow({
...
function prepareEnemyWorkflow(s, flow) {
  ...
  while (turn.enemyIndex < turn.enemyIds.length) {
    const enemy = s.enemies.find(
      (entry) => entry.id === turn.enemyIds[turn.enemyIndex],
    );
...
const RULE_ACTION_HANDLERS = {
  ...
  'scenario.werewolf.cure': (s, _command, action, context) => {
    ...
    infection.attempts++;
    const bonus = infection.attempts - 1 + cureBonus(s, context.hero, target),
      dice =
        context.dice ||
        roll(s, traitValue(context.hero, 'knowledge', s), {
          heroId: context.hero.id,
          label: context.hero.name + ' · 治疗狼毒',
          bonus,
        }),
      total = dice.reduce((sum, face) => sum + face, 0) + bonus;
    if (total >= WOLF_RULES.cureThreshold)
    ...
    announce(
      s,
      total >= WOLF_RULES.cureThreshold ? '狼毒已清除' : '治疗尚未成功',
      target.name +
        (total >= WOLF_RULES.cureThreshold
          ? '获得两轮净血保护。'
          : '仍有' + infection.turns + '轮；下次治疗额外 +1。'),
      'check',
      { dice, total, threshold: WOLF_RULES.cureThreshold },
    );
    log(
      s,
      context.hero.name +
  ...
  'scenario.werewolf.moonSeal': (s, _command, _action, context) => {
    const { hero, room } = context;
    hero.interacted = true;
    room.attempts++;
    const bonus = room.attempts - 1,
      dice =
        context.dice ||
        roll(
          s,
          modifierValue(
            ...
            traitValue(hero, 'knowledge', s),
          ),
          { heroId: hero.id, label: hero.name + ' · 净化月印', bonus },
        ),
      total = dice.reduce((sum, face) => sum + face, 0) + bonus,
      success = total >= WOLF_RULES.ritualThreshold;
    if (success) {
    ...
    announce(
      ...
      success
        ...
        : '下次尝试累计 +1。',
      'check',
      { dice, total, threshold: WOLF_RULES.ritualThreshold },
    );
  },
  ...
  'scenario.bells.seal': (s, _command, _action, context) => {
    const { hero, room } = context;
    hero.interacted = true;
    room.attempts++;
    const bonus = room.attempts - 1,
      dice =
        ...
        roll(
          ...
          modifierValue(
            s,
            'check.dice',
            { heroId: hero.id, checkKind: 'ritual', trait: 'knowledge' },
            traitValue(hero, 'knowledge', s),
          ),
          { heroId: hero.id, label: hero.name + ' · 知识', bonus },
        ),
      total = dice.reduce((sum, face) => sum + face, 0) + bonus,
...
function finishArrival(s) {
  ...
  if (!h.dead && destination) {
    const entered = enterRoom(s, h, destination, {
      rootActionId: `room-arrival:${p.uid}`,
      fromRoomId: p.fromRoomId,
      enterKind: p.enterKind || 'fall',
      discovered: !!p.discovered,
    });
    const destinationView = roomRuleView(s, destination, h.id);
    if (entered && destinationView.icon) drawCard(s, destinationView.icon, h);
  }
}
function advance(s) {
  ...
  if (p) {
    if (p.kind === 'placement') {
      ...
      if (a.type === 'place') {
        ...
        const placedView = roomRuleView(s, r, h.id);
        if (entered && placedView.icon) drawCard(s, placedView.icon, h);
        return s;
      }
      return s;
    }
...
export function createInteractiveGame(...args) {
  // Compatibility alias for existing UI and LAN callers.
...
function applyInteractiveAction(state, a) {
  ...
  if (registeredAction?.check) return startActionCheckWorkflow(state, a);
  ...
  if (state.phase === 'haunt' && ['endHero', 'endRound'].includes(a.type))
    return beginInteractiveEnemyTurn(state, a);
  return applyAction(state, a);
}
