...
function logFailure(event, error) {
  console.error('Remote room operation failed', {
    event,
    name: error?.name || 'Error',
    status: Number(error?.status) || 500,
    code: error?.code || errorCodeForStatus(Number(error?.status) || 500),
  });
}
...
export class GameRoom extends DurableObject {
  ...
  migrate() {
    ...
    this.sql.exec(`CREATE TABLE IF NOT EXISTS room_metrics (
      singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
      schema_version INTEGER NOT NULL,
      active_connections INTEGER NOT NULL DEFAULT 0,
      connection_high_water INTEGER NOT NULL DEFAULT 0,
      connections_total INTEGER NOT NULL DEFAULT 0,
      commands_total INTEGER NOT NULL DEFAULT 0,
      command_conflicts INTEGER NOT NULL DEFAULT 0,
      command_latency_total_ms INTEGER NOT NULL DEFAULT 0,
      command_latency_max_ms INTEGER NOT NULL DEFAULT 0,
      restore_failures INTEGER NOT NULL DEFAULT 0,
      updated_at INTEGER NOT NULL
    )`);
    this.sql.exec(
    ...
    this.sql.exec(
      `INSERT OR IGNORE INTO room_metrics (
    ...
    this.sql.exec(
      `UPDATE room_metrics SET schema_version = ?, updated_at = ?
      WHERE singleton = 1`,
      REMOTE_ROOM_SCHEMA_VERSION,
      Date.now(),
    );
  ...
  resetMetrics() {
    this.sql.exec(
      `UPDATE room_metrics SET
        schema_version = ?, active_connections = 0,
        connection_high_water = 0, connections_total = 0,
        commands_total = 0, command_conflicts = 0,
        command_latency_total_ms = 0, command_latency_max_ms = 0,
        restore_failures = 0, updated_at = ?
      WHERE singleton = 1`,
      REMOTE_ROOM_SCHEMA_VERSION,
      Date.now(),
    );
  ...
  syncConnectionMetrics(
    ...
    const active = Math.max(
      minimumActive,
      this.ctx.getWebSockets().filter((socket) => {
        if (socket === excludedSocket) return false;
        try {
          return typeof socket.deserializeAttachment()?.playerId === 'string';
        } catch {
          return false;
        }
      }).length,
    ...
    this.sql.exec(
      `UPDATE room_metrics SET
        active_connections = ?,
        connection_high_water = MAX(connection_high_water, ?),
  ...
  recordCommandMetric(startedAt, conflict = false) {
    const latency = Math.max(0, Date.now() - startedAt);
    this.sql.exec(
      `UPDATE room_metrics SET
        commands_total = commands_total + 1,
        command_conflicts = command_conflicts + ?,
        command_latency_total_ms = command_latency_total_ms + ?,
        command_latency_max_ms = MAX(command_latency_max_ms, ?),
        updated_at = ?
      WHERE singleton = 1`,
      conflict ? 1 : 0,
      latency,
  ...
  loadRoom({ allowExpired = false } = {}) {
    ...
    if (!room) {
      this.sql.exec(
        `UPDATE room_metrics SET restore_failures = restore_failures + 1,
    ...
    try {
      ...
      if (path === '/internal/command' && request.method === 'POST') {
        ...
        try {
          ...
          this.recordCommandMetric(startedAt);
          return json(response);
        } catch (error) {
          this.recordCommandMetric(startedAt, error?.status === 409);
          throw error;
        }
      }
      ...
      if (error instanceof RoomDomainError)
        return json(
          { error: error.message, code: errorCodeForStatus(error.status) },
          error.status,
        );
      logFailure('http_request', error);
      return json(
        { error: '远程房间暂时不可用，请稍后重试。', code: 'server_error' },
        500,
      );
    }
  }
  ...
  async handleWebSocketHello(socket, message) {
    ...
    const revision = loaded.room.revision,
      ...
      firstAuthentication = typeof previousAttachment?.playerId !== 'string',
      authenticatedBefore = this.ctx.getWebSockets().filter((candidate) => {
        if (candidate === socket) return false;
        try {
          return (
            typeof candidate.deserializeAttachment()?.playerId === 'string'
          );
        } catch {
          return false;
        }
    ...
    this.syncConnectionMetrics(
      firstAuthentication,
      authenticatedBefore + (firstAuthentication ? 1 : 0),
    );
  ...
  sendWebSocketError(socket, error, commandId) {
    ...
    if (status === 500) logFailure('websocket_message', error);
    try {
      this.send(
        socket,
  ...
  webSocketClose(socket, code, reason) {
    ...
    this.syncConnectionMetrics(false, 0, socket);
  }
