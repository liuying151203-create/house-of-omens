...
import { resolveGameSet } from '../game-sets.mjs';
...
function snapshot(room, player) {
  ...
  return {
    ...
    ...(room.gameSet ? { gameSet: structuredClone(room.gameSet) } : {}),
    game,
  };
}
...
export function createRoomState(data, { code, identity, now = Date.now } = {}) {
  ...
  let gameSet;
  if (data.gameSet != null) {
    try {
      gameSet = resolveGameSet(data.gameSet);
    } catch (error) {
...
export function restoreRoomState(value) {
  ...
  if (room.gameSet !== undefined) {
    try {
      room.gameSet = resolveGameSet(room.gameSet);
    } catch {
      return null;
    }
  }
