...
import { roomRuleView } from './rule-views.mjs';
import { factionOf } from './content/bloodmoon-mechanics.mjs';

export function roomBadges(game, room) {
  room = roomRuleView(game, room, game.active);
  const badges = [];
  if (
    ['bloodmoon-laboratory', 'bloodmoon-boiler', 'sterilization'].includes(
      room.special,
    )
  )
    badges.push({
  ...
  for (const trap of room.traps || [])
    if (trap.faction === factionOf(game.heroes[game.active]))
      badges.push({
        id: trap.id,
        icon: '⌁',
        label: '捕兽夹',
        description: '友方免触发；敌方进入时受到2点身体伤害并停止移动。',
      });
  if (
    game.bloodmoon?.steam?.some(
      (steam) => steam.active && steam.roomIds.includes(room.id),
    )
  )
    badges.push({
      id: 'steam',
      ...
      label: '蒸汽',
      description:
        '每片蒸汽持续期间，每人每房间首次进入或开始回合时进行速度4+；失败受1点身体伤害并停止移动。',
    });
  if (room.special === 'elevator')
    badges.push({
      ...
      label: '神秘电梯',
      description:
        '在人物行动中主动启动，掷 2 枚骰：0–1 地下室，2 一楼，3 二楼，4 任选楼层；0 点时室内每名探险者各受 1 枚骰肉体伤害。每次由操作者消耗1点移动力，同回合可重复启动；停止移动后不可用。房间连同乘员和标记搬移。没有合法落点则留在原处，仍消耗移动力。',
    });
